...
- `SPEC.md`: Unified, buildable specification (architecture, protocols, queue/state semantics).
...
- PRs should include: what/why, the impacted spec sections (`SPEC.md` headings), and the linked work item (`kanban/*.md` `id:` if applicable).
- If you change protocol/config/SQLite behavior, update `SPEC.md` (and `MVP.md` if it affects the MVP) and include a minimal verification checklist (commands + expected output/logs).
