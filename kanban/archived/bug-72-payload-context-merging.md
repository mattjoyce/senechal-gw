...
- 2026-02-12: **RESOLVED** - Dev team implemented Smart Core pattern: dispatcher automatically merges accumulated_json into event.payload before sending to plugins. Verified fix works: 3-hop pipeline completes successfully, output_dir accessible in final step, file written. Secondary issue discovered: file_handler prefers `content` over `result` field, causing wrong data to be written (484B original vs 1059B fabric analysis). Creating separate card #73 for field priority bug. (by @test-admin)
