...
## Decisions Locked (2026-02-24)

- Ingress mode: **Webhook relay** (Discord -> webhook -> Ductile).
- Publish mode: **Watch-triggered pipeline** (markdown write -> watch event -> Astro refresh).
- Accepted command syntax (v1): `/ai "<instruction>" <youtube_url>`
- Idempotency key source: Discord `request_id`.

## Input and Event Contract (v1)

Inbound webhook payload must include:

- `text` (raw Discord command content)
- `request_id` (Discord message/event id)
- `source=discord`
- `received_at` (RFC3339)

Internal routed event:

- `type`: `discord.youtube.summary.requested`
- `dedupe_key`: `discord:summary:<request_id>`
- `payload`:
  - `instruction_prompt`
  - `video_url`
  - `request_source=discord`
...
## Markdown Output Contract (v1)

- Output directory: `<astro_content_root>/summaries/`
- Filename: `<YYYY-MM-DD>-<youtube-video-id>-<slug>.md`
...
1. Webhook receives Discord command and emits `discord.youtube.summary.requested`.
2. Summary pipeline parses `/ai "<instruction>" <youtube_url>`.
3. Summarizer step generates structured summary content.
4. Writer step saves markdown atomically to Astro `summaries/`.
5. `folder_watch` emits `watch.folder.astro_content.changed`.
...
- [ ] The example command is accepted end-to-end:
  - `/ai "summaries the following, and out line practical learning and wisdom" https://youtu.be/XNzRIlcsOEQ?si=ibEyKoTz1uu8xSMP`
- [ ] Inbound webhook validates and normalizes `/ai "<instruction>" <youtube_url>` into `discord.youtube.summary.requested`.
- [ ] Duplicate deliveries with same `request_id` are no-op (no duplicate markdown).
- [ ] The pipeline produces one markdown file in configured Astro content path with valid frontmatter.
- [ ] Output markdown matches the v1 filename/frontmatter/body contract.
...
- [ ] `folder_watch` routes post-write change to Astro refresh pipeline.
...
- [ ] End-to-end run from example command results in updated RSS entry on `mattjoyce.ai`.
...
- 2026-02-24: Locked v1 implementation decisions for ingress/publish architecture, command/event schema, markdown contract, and idempotency strategy using Discord `request_id`. (by @assistant)
