...
*   If streaming or long-lived consumption capabilities become a critical need, the recommended approach would be to deploy a dedicated, external streaming system (e.g., a message queue, a separate service) that `ductile`'s existing spawn-per-command plugins can interact with (e.g., producing events to a queue, or having a dedicated `handle` command pull from a queue), thereby maintaining `ductile`'s architectural boundaries and focused scope.
